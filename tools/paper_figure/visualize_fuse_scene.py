...
if __name__ == '__main__':
    ...
    # result_fcgf_dir = '/home/ybbbbt/Developer/hybrid_feature/saved/0_paper_selected_3dmatch/3d_final/log_hf_1105_185250_H3DNetCoordVote_X3DMatchFragmentDataLoader_pos_neg_256_256_601_subset_round_2_epoch99_36dccd'
    ...
    # color = color_pca[0]
    color = map_to_color(desc2.max(1) ** 0.1, cmap='Spectral')
    # color = np.array([0.7, 0.7, 0.7])
    visualizer.add_np_points(kpt2, size=0.007, color=color, resolution=20, with_normal=True)
    ...
    color = map_to_color(scores1, cmap='Reds', vmin=0, vmax=1)
    rand1 = np.random.rand(scores1.shape[0])
    color = map_to_color(rand1, cmap='Paired')
    visualizer.add_np_points(kpt1, size=0.02, color=color, resolution=20, with_normal=True)
