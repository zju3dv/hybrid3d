...
class VoteFusionModuleV3(torch.nn.Module):
    def __init__(self, config):
        ...
        print('VoteFusionModuleV3 nms radius: ', self.nms_radius)
    ...
    def forward(self, input):
        ...
        if type(pcd_xyz) is torch.Tensor or self.conv_model_type == 'fcgf_official':
            ...
            pcd_conv_cache = input.get('pcd_conv_cache')
            if pcd_conv_cache is None:
                pcd_seed_features = self.pcd_conv(pcd_xyz).unsqueeze(0).transpose(1, 2) # [B, C, N]
            else:
                pcd_seed_features = pcd_conv_cache.unsqueeze(0).transpose(1, 2) # [B, C, N]
...
if __name__=='__main__':
    backbone_net = VoteFusionModuleV3(out_dim=64, config={
        'arch': {
            'balance_weight' : 0.5,
