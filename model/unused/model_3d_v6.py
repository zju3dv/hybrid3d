...
class H3DNet_v6(BaseModel):
    def __init__(self, layers=50, in_channels=4, pretrained=True):
        ...
        # c1, c2, c3, c4 = 64, 128, 256, 512 # 0.05s
        c1, c2, c3, c4 = 32, 64, 128, 256 # 0.005s
        self.layer_pcd_plain_conv = nn.Sequential(
            down(pcd_feat_channels, c1),
            down(c1, c2),
            down(c2, c3),
            down(c3, c4),
        )
        ...
        fuse_c = num_channels//2
        # fuse_c = num_channels//4
        self.conv2 = nn.Conv2d(num_channels//2 + c4,fuse_c,kernel_size=1)
        self.bn2 = nn.BatchNorm2d(fuse_c)
        ...
        self.decoder = make_deconv_layer(
            4,
            [fuse_c//2, fuse_c//4, fuse_c//8, fuse_c//16],
            [4, 4, 4, 4],
            fuse_c
        )
        # remove relu at last layer
        self.decoder = nn.Sequential(*list(self.decoder.children())[:-1])
        self.conv3_heatmap = nn.Conv2d(fuse_c//16,1,kernel_size=1)
