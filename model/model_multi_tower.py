...
class H3DMultiTower(BaseModel):
    def __init__(self, config=None):
        super(H3DMultiTower, self).__init__()
        use_leaky_relu = True
        ...
        self.pcd_heatmap_header = nn.Sequential(*list(filter(None.__ne__, [
            nn.Conv2d(pcd_c[-1], fuse_c, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(fuse_c),
            self.relu,
            self.dropout_decoder,
            nn.Conv2d(fuse_c, 1, kernel_size=3, stride=1, padding=1),
        ...
        self.rgb_heatmap_header = nn.Sequential(*list(filter(None.__ne__, [
            nn.Conv2d(rgb_c[-1], fuse_c, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(fuse_c),
            self.relu,
            self.dropout_decoder,
            nn.Conv2d(fuse_c, 1, kernel_size=3, stride=1, padding=1),
        ...
        self.pcd_heatmap_header.apply(weights_init)
        ...
        # self.pcd_method = 'fcgf'
        # self.pcd_method = 'd3feat'
    ...
    def forward(self, data, cache_pcd_inference=False):
        ...
        if self.pcd_method == 'pointnet2':
            ...
            pcd_3d_ftr = self.out_pcd_feat['features_keep_num']
        elif self.pcd_method == 'D3Feat':
        ...
        pcd_proj_ftr = F.interpolate(pcd_proj_ftr, scale_factor=0.5, mode='nearest', recompute_scale_factor=True)
        ...
        x_rgb_heatmap = self.rgb_heatmap_header(x_rgb) if self.output_multi_tower else None
        x_pcd_heatmap = self.pcd_heatmap_header(x_pcd) if self.output_multi_tower else None
        ...
        return {
            'depth' : None,
            'heatmap' : x_heatmap,
            'rgb_heatmap': x_rgb_heatmap,
            'pcd_heatmap': x_pcd_heatmap,
            'descriptor': x_desc,
