...
import numpy as np
from model.d3feat.utils.pointcloud import make_point_cloud
from functools import partial
import torch
import cpp_wrappers.cpp_subsampling.grid_subsampling as cpp_subsampling
import cpp_wrappers.cpp_neighbors.radius_neighbors as cpp_neighbors
from model.d3feat.utils.timer import Timer
...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    elif len(list_data[0]) == 2:
        datum = list_data[0]
        batched_points_list.append(datum[0])
        batched_features_list.append(datum[1])
        batched_lengths_list.append(len(datum[0]))
        sel_corr = np.array([])
        dist_keypts = np.array([])
