...
class DetLoss(nn.Module):
    ...
    def forward(self, dists, anc_score, pos_score):
        ...
        # rank loss
        pndiff = furthest_positive - closest_negative
        sigma = (anc_score + pos_score)
        rank_mask = ((pndiff.unsqueeze(1) - pndiff.unsqueeze(0)) > 0).float() * 2 - 1
        # pairwise distance of sigma
        sigma_pairwise_diff = sigma.unsqueeze(1) - sigma.unsqueeze(0)
        # pn_diff lower -> better pair
        loss = F.relu(sigma_pairwise_diff * rank_mask).mean()
