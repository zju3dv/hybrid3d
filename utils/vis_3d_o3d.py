...
class O3dVisualizer:
    ...
    def add_line_set(self, points, lines, colors=None):
        ...
        # )
        if colors is None: colors = [[random.uniform(0, 1), random.uniform(0, 1), random.uniform(0, 1)] for i in range(len(lines))]
        # line_set.colors = o3d.utility.Vector3dVector(colors)
        # self.geometries.append(line_set)
        mesh = LineMesh(points, lines, colors, radius=0.008)
        self.geometries.extend(mesh.cylinder_segments)

    def add_np_points(self, points, color=None, size=None, resolution=3, with_normal=False):
        if size == None:
            ...
            for idx, pt in enumerate(points):
                mesh_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=size, resolution=resolution)
                if with_normal:
