...
def main(config, cmd_args):
    ...
    checkpoint = torch.load('../KP2D/pretrained_models/v4.ckpt')
    ...
    model_type = 'heat_coord_desc' # or heat_coord_desc
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader, dynamic_ncols=True)):
            ...
            if config['data_loader']['read_frame_data']:
                ...
                for i_idx, indices in enumerate(batch_list):
                    ...
                    #     gray = input_rgb_np[i_frame]
                    score_1, coord_1, desc1 = keypoint_net(torch.from_numpy(input_rgb_np).float().cuda())
                        # output_descriptor.append(desc)
                        # output_heatmap.append(heatmap[None, ...])
                    output_descriptor = desc1
                    # output_descriptor = torch.from_numpy(output_descriptor).float().to(device)
                    output_heatmap = score_1
                    output_depth = None
