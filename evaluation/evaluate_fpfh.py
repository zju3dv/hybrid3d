...
def main(config, cmd_args):
    ...
    use_fpfh = True
    if use_fpfh:
        config['data_loader']['read_frame_data'] = False
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader, dynamic_ncols=True)):
            ...
            if hasattr(model, 'forward_vote'):
                if use_fpfh:
                    ...
                    pcd_np = pcd_data.pos.numpy().squeeze()
                    pcd.points = o3d.utility.Vector3dVector(pcd_np)
                    pcd.estimate_normals()
                    pcd_fpfh = o3d.registration.compute_fpfh_feature(
                        pcd, o3d.geometry.KDTreeSearchParamRadius(0.15))
                    clustered_descriptors = np.asarray(pcd_fpfh.data).T
                    clustered_kpts_weights = np.concatenate([pcd_np, np.random.rand(pcd_np.shape[0])[:, None]], axis=1)
                else:
